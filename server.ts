...
import { INITIAL_VEGETABLES, generateHistoryForVegetable } from './src/data/vegetablesData';
import { VegetablePrice, PriceHistoryPoint, SMSAlertRule, SMSLog, FitSMSConfig } from './src/types';
...
// Sri Lanka Phone Number Formatter supporting single or multiple numbers
function parseMultipleSriLankaPhones(phoneInput: string): string[] {
  ...
  for (let part of rawParts) {
    ...
    // Convert 0762755190 (10 digits starting with 0) to 94762755190
...
function formatSriLankaPhone(phone: string): string {
  const phones = parseMultipleSriLankaPhones(phone);
  return phones.join(',');
}

// In-memory SMS rules and logs
...
async function sendFitSMS(rawRecipient: string, message: string, tokenOverride?: string, senderIdOverride?: string) {
  ...
  // FitSMS accepts string or array for multi-recipients (formatted as 9476XXXXXXX)
  const payload = {
...
// Dambulla Dedicated Economic Center Product ID Mapping (from https://dambulladec.com / https://api.dambulladec.com)
...
// Helper function to sync live prices from dambulladec.com
...
// Initial live background sync on server startup
...
async function startServer() {
  ...
  // 2. Vegetable History API (Fetches Real Historical Chart Points from dambulladec.com)
  app.get('/api/prices/history', async (req, res) => {
    ...
    // Fallback if real history fetch failed or unavailable
  ...
  // 3. SMS Alerts & FitSMS Gateway Endpoints
  ...
  // Test SMS API Trigger (Supports FitSMS Live API + Simulated Gateways)
...
Ensure valid JSON output without extra markdown code block syntax if possible.`;
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Dambulla DEC Vegetable Price Server running on http://localhost:${PORT}`);
  });
}
